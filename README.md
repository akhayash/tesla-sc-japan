...
周辺施設として、OpenStreetMapのコンビニ・道の駅・ショッピングモールを種類ごとに重ねられます（初期状態はOFF）。引いた表示では小さな点、拡大すると種類別のアイコンで描画し、コンビニは市街地レベルから表示します。コンビニはチェーンごとに色と頭文字（7・F・L など）で区別します。施設を選択すると、充電器と同じく航空写真（拡大ビューアー付き）とストリートビュー／Googleマップへのリンクを表示します。
